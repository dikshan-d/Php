...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_REQUEST['bkid'])) {
		$cancelsub = "Package cancellation";
		$cancelmessage = "Hello,<br>"
		...
		$sql_email = "SELECT UserEmail FROM tblbooking WHERE BookingId=:bid";
		$query_email = $dbh->prepare($sql_email);
		$query_email->bindParam(':bid', $bid, PDO::PARAM_STR);
		$query_email->execute();
		$email_result = $query_email->fetch(PDO::FETCH_ASSOC);
		$email = $email_result['UserEmail'];

		$mail = new PHPMailer(true);
		$mail->isSMTP();
		$mail->Host = 'smtp.gmail.com';
		$mail->SMTPAuth = true;
		$mail->Username = 'dikchyand@gmail.com';
		$mail->Password = 'cwrnvuqeqbjllwlx';
		$mail->Port = 465;
		$mail->SMTPSecure = 'ssl';
		$mail->isHTML(true);
		$mail->setFrom('noreply@gmail.com');
		$mail->addAddress($email);
		$mail->Subject = ($cancelsub);
		$mail->Body = $cancelmessage;
		$mail->send();
	...
	if (isset($_REQUEST['bckid'])) {
		$confirmsub = "Package Confirmation";
		$confrimmessage = "Hello,<br>"
		...
		// Get email associated with bcid
		$sql_email = "SELECT UserEmail FROM tblbooking WHERE BookingId=:bcid";
		$query_email = $dbh->prepare($sql_email);
		$query_email->bindParam(':bcid', $bcid, PDO::PARAM_STR);
		$query_email->execute();
		$email_result = $query_email->fetch(PDO::FETCH_ASSOC);
		$email = $email_result['UserEmail'];
		...
		$mail->setFrom('noreply@gmail.com');
		$mail->addAddress($email);
		$mail->Subject = ($confirmsub);
		$mail->Body = $confrimmessage;
		$mail->send();
