...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		if (!empty($pname) and !empty($ptype) and !empty($plocation) and !empty($pprice) and !empty($pfeatures) and !empty($pdetails)) {
			if ($pprice > 100) {
				...
				$error = "Price cannot be less than Rs.100";
			}
		}
	}
								...
								if ($query->rowCount() > 0) {
									foreach ($results as $result) {	?>
										...
										<form class="form-horizontal" name="package" method="post" enctype="multipart/form-data">
											...
											<div class="form-group">
												<label for="focusedinput" class="col-sm-2 control-label">Package Price in NRP</label>
												<div class="col-sm-8">
													<input type="text" class="form-control1" name="packageprice" id="packageprice" placeholder=" Package Price in NRP" value="<?php echo htmlentities($result->PackagePrice); ?>" required>
												</div>
											</div>
